...
def test_start_team_process_happy(reset_team_state, monkeypatch, tmp_path) -> None:
    ...
    assert len(app_module._get_team_state_for_role("managing-director")["starts_history"]) == 1
...
def test_stop_team_process_happy(reset_team_state, monkeypatch, tmp_path) -> None:
    ...
    assert app_module._get_team_state_for_role("managing-director")["process"] is None
...
def test_team_status_running(reset_team_state) -> None:
    ...
    fake.pid = 1234
    app_module._get_team_state_for_role("managing-director")["process"] = fake
    app_module._get_team_state_for_role("managing-director")["started_at"] = 1700000000
...
def test_has_pending_work_empty(tmp_path, monkeypatch) -> None:
    ...
    # D38BCDDA9CF9: используем _has_pending_work_for_role для managing-director
    assert app_module._has_pending_work_for_role("managing-director") is False
...
def test_has_pending_work_todo_for_team_lead(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(app_module, "DB_PATH", db_path)
    # D38BCDDA9CF9: проверяем работу для dev-lead (legacy: тимлид)
...
def test_auto_can_start_disabled(reset_team_state) -> None:
    app_module._global_state["auto_mode"] = False
    ok, reason = app_module._auto_can_start_for_role("managing-director", 1000)
    assert ok is False
...
def test_auto_can_start_rate_limit_min_interval(reset_team_state) -> None:
    ...
    app_module._get_team_state_for_role("managing-director")["starts_history"] = [now - 5]
    ok, reason = app_module._auto_can_start_for_role("managing-director", now)
    assert ok is False
...
def test_auto_can_start_rate_limit_per_hour(reset_team_state) -> None:
    ...
    app_module._get_team_state_for_role("managing-director")["starts_history"] = [now - 60 * i for i in range(20)]
    ok, reason = app_module._auto_can_start_for_role("managing-director", now)
    assert ok is False
...
def test_auto_can_start_cleans_old_history(reset_team_state, tmp_path, monkeypatch) -> None:
    ...
    app_module._get_team_state_for_role("managing-director")["starts_history"] = [now - 7200, now - 5000]
    ok, _ = app_module._auto_can_start_for_role("managing-director", now)
    assert ok is True
    # старые метки вычищены
    assert app_module._get_team_state_for_role("managing-director")["starts_history"] == []
...
def test_auto_can_start_blocked_while_reader_alive(reset_team_state, tmp_path, monkeypatch) -> None:
    ...
    live_thread.start()
    app_module._get_team_state_for_role("managing-director")["reader_thread"] = live_thread
    ...
    ok2, reason2 = app_module._auto_can_start_for_role("managing-director", 1_000_000)
    assert ok2 is True
    assert reason2 == "ок"
...
def test_auto_can_start_ok_when_reader_finished(reset_team_state, tmp_path, monkeypatch) -> None:
    ...
    app_module._get_team_state_for_role("managing-director")["reader_thread"] = dead_thread
...
def test_start_team_process_sets_reader_thread(reset_team_state, monkeypatch, tmp_path) -> None:
    ...
    assert res["ok"] is True
    t = app_module._get_team_state_for_role("managing-director")["reader_thread"]
...
def test_auto_mode_restart_does_not_start_while_reader_cleanup(
    ...
    app_module._get_team_state_for_role("managing-director")["process"] = dead_proc

    # reader_thread всё ещё жив (cleanup)
    cleanup_event = threading.Event()
    reader = threading.Thread(target=cleanup_event.wait, daemon=True)
    reader.start()
    app_module._get_team_state_for_role("managing-director")["reader_thread"] = reader

    # Пока reader жив — старт заблокирован
    ok, reason = app_module._auto_can_start_for_role("managing-director", 2_000_000)
    assert ok is False
    assert "reader_thread" in reason
    ...
    ok2, reason2 = app_module._auto_can_start_for_role("managing-director", 2_000_000)
    assert ok2 is True
...
# === _parse_and_record_session_from_log ===


def test_parse_log_records_final_result_event(tmp_path, monkeypatch) -> None:
    """Парсер находит последний result-event в stream-json логе и пишет в БД."""
    from devboard_tasks import db as _db
    db_path = tmp_path / "x.db"
    _db.init_db(db_path)
    monkeypatch.setattr(app_module, "DB_PATH", db_path)

    log_path = tmp_path / "planning-test.log"
    lines = [
        "==== dev-lead round 1 ====",
        json.dumps({"type": "system", "subtype": "init"}),
        json.dumps({"type": "stream_event", "event": "message_start"}),
        json.dumps({
            "type": "result", "subtype": "success",
            "duration_ms": 8000, "num_turns": 3,
            "total_cost_usd": 0.123, "model": "claude-sonnet-4-6",
    ...
    cost = app_module._parse_and_record_session_from_log(log_path)
    assert cost == pytest.approx(0.123)

    # В claude_sessions появилась запись с правильным cost
    summary = _db.usage_summary(db_path)
    # сессия в last_5h окне (только что создана)
    assert summary["last_5h"]["sessions"] == 1
    assert summary["last_5h"]["cost_usd"] == pytest.approx(0.123)


def test_parse_log_picks_last_result_when_multiple(tmp_path, monkeypatch) -> None:
    """Если в логе несколько result-events (несколько раундов в одном файле),
    парсер берёт самый последний."""
    from devboard_tasks import db as _db
    db_path = tmp_path / "x.db"
    _db.init_db(db_path)
    monkeypatch.setattr(app_module, "DB_PATH", db_path)

    log_path = tmp_path / "planning-multi.log"
    lines = [
        json.dumps({"type": "result", "duration_ms": 1000,
                    "total_cost_usd": 0.10, "is_error": False}),
        "==== marketing-lead round 1 ====",
        json.dumps({"type": "result", "duration_ms": 2000,
                    "total_cost_usd": 0.25, "is_error": False}),
    ]
    log_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    cost = app_module._parse_and_record_session_from_log(log_path)
    assert cost == pytest.approx(0.25)  # последний result


def test_parse_log_missing_file_returns_none(tmp_path) -> None:
    """Парсер не падает на отсутствующем файле."""
    result = app_module._parse_and_record_session_from_log(tmp_path / "nope.log")
    assert result is None


def test_parse_log_no_result_event_returns_none(tmp_path, monkeypatch) -> None:
    """Если result-event нет в логе — возвращает None, ничего не пишет в БД."""
    from devboard_tasks import db as _db
    db_path = tmp_path / "x.db"
    _db.init_db(db_path)
    monkeypatch.setattr(app_module, "DB_PATH", db_path)

    log_path = tmp_path / "planning-noresult.log"
    log_path.write_text(
        json.dumps({"type": "system", "subtype": "init"}) + "\n"
        "not json line\n"
