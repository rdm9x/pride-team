...
> **Примечание по кириллице**: Сами файлы проекта (roles/тимлид.md и др.) содержат кириллические имена — это намеренно и корректно обрабатывается через `encoding="utf-8"` во всех read/write операциях. Проблема только в **пути к директории проекта** на некоторых Windows-конфигурациях.
