...
это делает `commands/pride-team-work.sh`). Это самый дешёвый путь к
...
* `permission_mode` — `bypassPermissions` / `default` / `acceptEdits`.
  В headless-режиме pride-team нужен `bypassPermissions` (approval-gates
...
class ClaudeCLIProvider(LLMProvider):
    ...
    async def invoke(
        ...
        «CLI ≠ полноценный диалог»). Для штатных pride-team сценариев
