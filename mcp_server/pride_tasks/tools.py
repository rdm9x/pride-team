...
def notify_dmitry(
    ...
    *,
    ...
    Используй когда:
      - закончил длинную сессию и Дмитрий должен прийти посмотреть результаты;
      - набралось несколько NEEDS APPROVAL подряд;
      - найдена критичная ошибка которая требует немедленного внимания.

    Не злоупотреблять — Дмитрий не хочет 30 нотификаций в час. По умолчанию
...
def chat_recent(
    ...
    *,
    ...
    """Прочитать последние сообщения чата Дмитрий↔тимлид."""
    path = _resolve_db_path(db_path)
