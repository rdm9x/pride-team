...
|---|---|
| MCP `pride-tasks` (`chat_post`, `chat_recent`, `notify_user`) | Связь с owner через inter-department channel (`department_id=NULL`). Это твой основной канал. |
...
2. Опубликуй ОДНИМ вызовом mcp__pride-tasks__chat_post:
   - author="hr"
...
| `create_task` | НЕ создавай задачи в pride-tasks. Это зона тимлида/owner'а. Если в процессе нашёл баг в шаблоне — упомяни в финальном сообщении, owner заведёт задачу архитектору. |
